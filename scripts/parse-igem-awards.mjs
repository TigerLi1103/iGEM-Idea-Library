...
for (const rawLine of lines) {
  ...
  if (section === 'grand') {
    ...
    if (m) {
      const [, divCode, level, teamRaw, _country, village] = m;
  ...
  if (section === 'village') {
    ...
    if (m) {
      ...
      for (const teamName of [...new Set([...winners, ...nominees])]) {
        ...
        if (!entry.village && awardName.startsWith('Best ')) {
          entry.village = awardName.replace(/^Best\s+/, '').replace(/\s+Project$/, '').trim();
        }
      }
    }
    continue;
...
const entries = Array.from(yearData.values())
  .map(entry => {
    ...
    entry.specialPrizes = entry.specialPrizes.slice(0, 3);
